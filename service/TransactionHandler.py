    ...
    def transaction(self, text_str, json_info):
        ...
        if re_match:
            for matchNum, match in enumerate(matches, start=1):
                ...
                transaction["date"] = fromtimestamp.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def save_transactions(self, transactions, headers):
        for transaction in transactions:
            ...
            if installments_ > 1 and transaction["is_installment"] == "S":
                ...
                transactions_ = self.remote_repository.get_objects(
                    "transactions", data=filter_, headers=headers
                )
                for transaction_ in transactions_:
                    ...
                    self.remote_repository.insert(
                        "transactions", data=transaction_, headers=headers
                    )
                    suffix = f" 1/{installments_}"
                    if transaction_["text"].endswith(suffix):
                        self.check_and_save_cashback(
                            transaction_, num_parcs=installments_, headers=headers
                        )
        ...
        try:
            ...
            if is_new:
                try:
                    if (
                        "key" in transaction
                        and transaction["key"] is not None
                        and transaction["key"].startswith("ia_extractor_")
                    ):
                        ...
                        if exists.__len__() > 0:
                            self.logger.info(
                                f"Transaction already saved-> {transaction['key']} -> {transaction}"
                            )
                    ...
                    self.logger.exception(e)
            response = self.remote_repository.insert(
                "transactions", data=transaction, headers=headers
            )
    ...
    def check_and_save_cashback(self, transaction, num_parcs=1, headers=None):
        ...
        if "key" in transaction and transaction["key"] is not None:
            try:
                ...
                exists_cashback = self.remote_repository.get_objects(
                    "transactions", data={"key": cashback_key}, headers=headers
                )
                if exists_cashback:
        ...
        if (
            not cashback_exists
            and transaction.get("app_name") == "Nubank"
            and transaction.get("type") == "outcome"
            and transaction.get("category") != "Ignored"
            and num_parcs > 0
        ):
            try:
                ...
                cashback_transaction["text"] = (
                    f"Cashback de {cashback_value} referente a {original_text}"
                )

                if (
                    "key" in cashback_transaction
                    and cashback_transaction["key"] is not None
                ):
                    cashback_transaction["key"] = (
                        f"{cashback_transaction['key']}_cashback"
                    )
    ...
    def process_b3_dividends(self, file_storage, headers):
        ...
        if headers:
            try:
                ...
                self.logger.warning(
                    f"Could not retrieve user from headers, using default user_id = 1. Error: {e}"
                )
        ...
        for r in range(2, max_r + 1):
            ...
            if len(exists) == 0:
                ...
                self.remote_repository.insert(
                    "dividends_b3", data=dividend_data, headers=headers
                )
    ...
    def process_nubank_ofx(self, file_storage, headers):
        ...
        for block in stmt_blocks:
            ...
            if len(exists) == 0:
                ...
                transaction_ia = {"id": key, "descricao": text}
                ...
                for resp_ai in resps_ai:
                    ...
                    for t in transactions:
                        if t["key"] == id_ai:
                            ...
                            self.remote_repository.insert(
                                "transactions", data=t, headers=headers
                            )
                            inserted_count += 1
                            break
        ...
        try:
            ...
            if _url_bff:
                ...
                try:
                    ...
                    self.logger.error(
                        f"[{event_id}] Erro ao decodificar JSON: {_response.text}"
                    )
